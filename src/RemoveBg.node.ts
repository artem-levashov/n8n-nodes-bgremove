...
async function rembgV2(input: Buffer, format: 'png'|'jpeg'|'webp'): Promise<Buffer> {
  const { Rembg } = require('rembg-node');
  const sharp = require('sharp');
  if (!Rembg) throw new Error('rembg-node v2 not found (Rembg missing)');
  const rembg = new Rembg({ logging: false });
  const shIn = sharp(input);
  const shOut = await rembg.remove(shIn);
  if (format === 'png')  return await shOut.png().toBuffer();
  if (format === 'jpeg') return await shOut.jpeg().toBuffer();
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        const engine = this.getNodeParameter('engine', i, 'rembg') as 'rembg'|'tb'|'auto';
        ...
        const tryRembg = async () => {
          try { output = await rembgV2(input, format); usedFormat = format; dbg.tries.push({ engine: 'rembg', ok: true }); }
          catch (e:any) { dbg.tries.push({ engine: 'rembg', ok: false, err: e?.message || String(e) }); }
        };
        const tryTb = async () => {
          try { const mod = require('transparent-background'); output = await tbOnce(mod, input, format, { fast }); usedFormat = format; dbg.tries.push({ engine: 'tb', ok: true, fast }); }
          catch (e:any) { dbg.tries.push({ engine: 'tb', ok: false, err: e?.message || String(e) }); }
        };

        if (engine === 'rembg') await tryRembg();
        else if (engine === 'tb') await tryTb();
        else { await tryRembg(); if (!hasData(output)) await tryTb(); }
